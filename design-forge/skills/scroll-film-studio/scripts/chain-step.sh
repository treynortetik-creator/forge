#!/bin/zsh
# chain-step.sh <assets-dir> <clip-name> <start-image> <end-image> <prompt> [resolution] [prev-last-png]
#
# Generates one Higgsfield Seedance clip pinned to both endpoint images, waits, downloads,
# extracts its first and last frames, and SSIM-gates the junction against <prev-last-png>.
...
set -e -o pipefail
DIR=$1; NAME=$2; START=$3; END=$4; PROMPT=$5; RES=${6:-1080p}; PREV=${7:-}
if [[ -z "$DIR" || -z "$NAME" || -z "$START" || -z "$END" || -z "$PROMPT" ]]; then
  echo "usage: chain-step.sh <assets-dir> <clip-name> <start-image> <end-image> <prompt> [resolution] [prev-last-png]"; exit 1
fi
[[ "$NAME" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ && "$NAME" != *..* ]] || {
  echo "invalid clip name: $NAME"; exit 1
}
...
[[ "$RES" == "480p" || "$RES" == "720p" || "$RES" == "1080p" ]] || {
  echo "resolution must be 480p, 720p or 1080p"; exit 1
}
for bin in higgsfield ffmpeg ffprobe python3 curl; do
